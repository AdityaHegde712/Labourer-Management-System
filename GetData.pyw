...
class Application(Frame):
    ...
    def callback_daily(self):
        script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "TotalDaily.pyw")
        date_range = self.date_range(self.date1.get_date(), self.date2.get_date())
        command = ["pythonw", script_path] + list(date_range)
        subprocess.run(command)

    def callback_monthly(self):
        script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "TotalMonthly.pyw")
        date_range = self.date_range(self.date1.get_date(), self.date2.get_date())
        command = ["pythonw", script_path] + list(date_range)
        subprocess.run(command)
    
    def callback_final(self):
        script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "FinalCost.pyw")
        date_range = self.date_range(self.date1.get_date(), self.date2.get_date())
        command = ["pythonw", script_path] + list(date_range)
        subprocess.run(command)
    
    def callback_attendance(self):
        script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "GetAttendance.pyw")
        date_range = self.date_range(self.date1.get_date(), self.date2.get_date())
        command = ["pythonw", script_path] + list(date_range)
        subprocess.run(command)
